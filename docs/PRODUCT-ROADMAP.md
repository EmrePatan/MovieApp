...
## DONE — Discovery 2.0 D4 (On TV This Week)

General TV discovery for shows with episodes airing soon — **not** personalized Coming Up / followed scope.

**Product definition ("This Week"):**
- Shows with at least one episode airing in TMDB's rolling **next 7 days** window (episode air-date activity, not show first-air date)
- TMDB defines this via the canonical **On The Air** list; MovieApp does **not** compute calendar Mon–Sun boundaries or apply client-side week filtering

**Backend:**
- `GET /api/discovery/on-tv-this-week?page=1&pageSize=20` — returns standard `SearchResponse` **TV-only** items; unauthenticated
- TMDB source: **`tv/on_the_air`** with `page={page}` only (region-neutral; no `releaseRegion` / `watchRegion` / timezone selector)
- Cache key `discovery-on-tv-this-week:{page}:{pageSize}:v1`; TTL 30 minutes
- Lazy summary ingestion via `EnsureFromSummariesAsync`; no schema change
- Distinct from `TvUpcomingEpisodeSync` / Coming Up (followed, personalized episode scheduling)
...
Origin-country discovery for movies and TV — **content origin**, not streaming or theatrical availability.

**Semantics:**
- `originCountry` = ISO 3166-1 alpha-2 content origin (`with_origin_country` via D1 Advanced Discover)
- Distinct from `watchRegion` (D2 streaming availability) and `releaseRegion` (D3 theatrical availability)
- Not inferred from `originalLanguage` alone
...
**Discovery filter navigation:**
- Filter / sort / region / media-type mutations on Advanced Discover, Streaming Services, Now in Theaters, and World Cinema use `router.setParams` to replace current URL state without adding navigation-history entries
- Real screen navigation (Discover → full screens, result → detail, detail → person) remains normal `push` / `dismissTo` behavior
- Detail return hrefs still preserve the current filtered route state

**Where to Watch:**
- Detail-page `WhereToWatchRail` uses User Region (with hydration gating) instead of hardcoded `TR`
- Opening detail from Streaming Services (or Advanced Discover with active streaming filters) passes temporary `watchRegion` context so Where to Watch matches the exploration region without mutating global User Region
...
- Discover hub with D1 **Explore with Filters**, D2 **Streaming Services**, D5 **World Cinema** preview, D3 **Now in Theaters** preview, D4 **On TV This Week** preview, future D6 placeholder (non-interactive), and existing Trending / Top Rated / genre browse
